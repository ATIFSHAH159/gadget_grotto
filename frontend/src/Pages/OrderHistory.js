...
import { getMyOrders, getAllOrders, updateOrderStatus } from '../Services/api';
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = user.isAdmin ? await getAllOrders() : await getMyOrders();
  ...
  return (
    <div className="auth-container">
      <div className="auth-form" style={{ textAlign: 'center', maxWidth: 1000 }}>
        <h2 style={{ color: '#00FFE5', marginBottom: 20 }}>Order History</h2>
        {loading ? (
            ...
            <table style={{ width: '100%', color: '#fff', borderCollapse: 'collapse', minWidth: 900 }}>
              <thead>
                <tr style={{ color: '#00FFE5', borderBottom: '1px solid #2A2A30' }}>
                  ...
                  {orders[0]?.user && (<th style={{ padding: 8 }}>User</th>)}
                  ...
                  <tr key={order._id} style={{ borderBottom: '1px solid #2A2A30', verticalAlign: 'top' }}>
                    ...
                    {order.user && (
                      <td style={{ padding: 8, fontSize: 13 }}>
                        ...
                        <div style={{ color: '#B0B0B8', fontSize: 12 }}>{order.user.email}</div>
                      </td>
                    )}
                    ...
                    <td style={{ padding: 8, color: order.status === 'Paid' ? '#00FFE5' : '#FF3B5B', fontWeight: 500 }}>{order.status || (order.isPaid ? 'Paid' : 'Pending')}</td>
                    <td style={{ padding: 8 }}>
                      {JSON.parse(localStorage.getItem('user'))?.isAdmin ? (
                        ...
                        <span style={{ color: '#00FFE5', fontWeight: 500 }}>{order.deliveryStatus || 'Pending'}</span>
                      )}
                    </td>
                    <td style={{ padding: 8 }}>
