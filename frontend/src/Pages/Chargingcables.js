...
function ChargingCables() {
  ...
  const getProductDetail = async (category) => {
    try {
      ...
      setCableData(result.data);
    } catch (error) {
      console.log("Error ", error);
    }
  };

  const filteredData = cableData.filter((item) => item.price <= sliderValue);
  ...
  return (
    <div className="product-page">
      <div className="product-header">
        <Container>
          <h1 className="product-title">Charging Cables</h1>
          <p className="product-subtitle">High-quality charging cables for all your devices</p>
        </Container>
      </div>
        ...
        <div className="product-grid">
          ...
          {filteredData.length === 0 ? (
            <div className="no-products">
              ...
              <p>We couldn't find any charging cables matching your filter criteria. Try adjusting your filters.</p>
            </div>
          ) : (
            <Row className="g-4">
