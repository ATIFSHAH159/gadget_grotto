...
function PowerBanks() {
  ...
  const getProductDetail = async (category) => {
    try {
      ...
      setpowerbankdata(result.data);
    } catch (error) {
      console.log("Error ", error);
    }
  };

  const filteredData = powerbankdata.filter((item) => item.price <= sliderValue);
  ...
  return (
    <div className="product-page">
      <div className="product-header">
        <Container>
          <h1 className="product-title">Power Banks</h1>
          <p className="product-subtitle">Keep your devices charged on the go with our premium power banks</p>
        </Container>
      </div>
        ...
        <div className="product-grid">
          ...
          {filteredData.length === 0 ? (
            <div className="no-products">
              ...
              <p>We couldn't find any power banks matching your filter criteria. Try adjusting your filters.</p>
            </div>
          ) : (
            <Row className="g-4">
