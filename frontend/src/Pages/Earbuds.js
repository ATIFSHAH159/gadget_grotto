...
function EarBuds() {
  ...
  const getProductDetail = async (category) => {
    try {
      ...
      setEarbudsData(result.data);
    } catch (error) {
      console.log("Error ", error);
    }
  };

  const filteredData = earbudsData.filter((item) => item.price <= sliderValue);
  ...
  return (
    <div className="product-page">
      <div className="product-header">
        <Container>
          <h1 className="product-title">Earbuds</h1>
          <p className="product-subtitle">Experience superior sound quality with our premium wireless earbuds</p>
        </Container>
      </div>
        ...
        <div className="product-grid">
          ...
          {filteredData.length === 0 ? (
            <div className="no-products">
              ...
              <p>We couldn't find any earbuds matching your filter criteria. Try adjusting your filters.</p>
            </div>
          ) : (
            <Row className="g-4">
