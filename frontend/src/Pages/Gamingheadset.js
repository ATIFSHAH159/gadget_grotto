...
function GamingHeadsets() {
  const [gamingHeadsetData, setGamingHeadsetData] = useState([]);
  const [showPriceFilter, setShowPriceFilter] = useState(true);
  const [filterCollapsed, setFilterCollapsed] = useState(false);

  const { sliderValue, handleSliderChange, addToCart } = useContextData();
  const category = "Gaming Headsets";
  ...
  const getProductDetail = async (category) => {
    try {
      ...
      setGamingHeadsetData(result.data);
    } catch (error) {
      console.log("Error ", error);
    }
  };

  const filteredData = gamingHeadsetData.filter((item) => item.price <= sliderValue);
  ...
  return (
    <div className="product-page">
      <div className="product-header">
        <Container>
          <h1 className="product-title">Gaming Headsets</h1>
          <p className="product-subtitle">Immerse yourself in your games with our high-performance gaming headsets</p>
        </Container>
      </div>
        ...
        <div className="product-grid">
          ...
          {filteredData.length === 0 ? (
            <div className="no-products">
              ...
              <p>We couldn't find any gaming headsets matching your filter criteria. Try adjusting your filters.</p>
            </div>
          ) : (
            <Row className="g-4">
