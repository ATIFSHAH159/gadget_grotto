...
function PhoneCases() {
  ...
  const category = "Phone Cases & Covers";
  ...
  const getProductDetail = async (category) => {
    try {
      ...
      setPhoneCaseData(result.data);
    } catch (error) {
      console.log("Error ", error);
    }
  };

  const filteredData = phoneCaseData.filter((item) => item.price <= sliderValue);
  ...
  return (
    <div className="product-page">
      <div className="product-header">
        <Container>
          <h1 className="product-title">Phone Cases & Covers</h1>
          <p className="product-subtitle">Protect your device with our stylish and durable phone cases</p>
        </Container>
      </div>
        ...
        <div className="product-grid">
          ...
          {filteredData.length === 0 ? (
            <div className="no-products">
              ...
              <p>We couldn't find any phone cases matching your filter criteria. Try adjusting your filters.</p>
            </div>
          ) : (
            <Row className="g-4">
