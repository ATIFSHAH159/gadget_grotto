...
function MobileSkins() {
  ...
  const getProductDetail = async (category) => {
    try {
      ...
      setMobileSkinData(result.data);
    } catch (error) {
      console.log("Error ", error);
    }
  };

  const filteredData = mobileSkinData.filter((item) => item.price <= sliderValue);
  ...
  return (
    <div className="product-page">
      <div className="product-header">
        <Container>
          <h1 className="product-title">Mobile Skins</h1>
          <p className="product-subtitle">Personalize your device with our stylish and durable mobile skins</p>
        </Container>
      </div>
        ...
        <div className="product-grid">
          ...
          {filteredData.length === 0 ? (
            <div className="no-products">
              ...
              <p>We couldn't find any mobile skins matching your filter criteria. Try adjusting your filters.</p>
            </div>
          ) : (
            <Row className="g-4">
